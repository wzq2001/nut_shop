        ...
        <template #bodyCell="{ text, record, index, column }">
          <template v-if="column.key === 'status'">
            <a-tag :color="text === '1'? '#2db7f5':'#87d068'">
              {{text===  '1'? '待支付': text === '2'? '待发货': text === '3'? '待收货': text === '4'? '已收货':'已取消'}}
            </a-tag>
          </template>
          <template v-if="column.key === 'operation'">
            <span>
              ...
              <a-popconfirm title="确定发货?" ok-text="是" cancel-text="否" @confirm="confirmUpdate(record)">
                <a>发货</a>
              </a-popconfirm>
...
<script setup lang="ts">
  ...
  import { createApi, listApi, updateApi, deleteApi, cancelApi,updateOrderApi } from '/@/api/order';
  ...
  const confirmUpdate = (record: any) => {
    updateOrderApi({id: record.id,status: "3"})
        .then((res) => {
          getDataList();
